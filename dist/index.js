...
facebookScraper.on("progress", (progress) => {
  console.log(
    `[Facebook Scraper] Progress: ${progress.percentage}% (${progress.current}/${progress.total})`
  );
});
...
whatsappBot.on("progress", (progress) => {
  console.log(
    `[WhatsApp Bot] Scan progress: ${progress.percentage}% (${progress.current}/${progress.total})`
  );
});
// Basic API endpoints
...
app.get("/api/test", async (req, res) => {
  try {
    ...
    res.json({
      ...
      whatsappStatus: whatsappBot.isReady()
        ? "connected"
        : "waiting for QR scan",
...
app.post("/api/whatsapp/auth", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "Authentication process started. Check your WhatsApp for verification code.",
    });
  } catch (error) {
